...
col3.metric("Test Acc", f"{status['test_acc']*100:.1f}%", f"Güvən: {status['model_agreement']*100:.0f}%")
...
fig.update_layout(height=650, xaxis_rangeslider_visible=False, showlegend=False, margin=dict(l=10,r=10,t=10,b=10))
...
with colA:
    ...
    st.subheader("📅 Təqvim")
    st.code(get_calendar_text(), language="text")
        ...
        try:
            ...
            if not df_log.empty:
                ...
                if not closed.empty:
                    ...
                    st.metric("WinRate Son 20", f"{wins/len(closed)*100:.1f}% ({wins}/{len(closed)})")
            else:
                st.info("Hələ siqnal yoxdur")
        except Exception as e:
            st.warning(f"Log oxunmadı: {e}")
    else:
        st.info("Hələ signals_log.csv yaranmayıb")
